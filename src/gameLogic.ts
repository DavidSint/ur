...
const blackOutboundPath: Position[] = [
  "b0",
  "b1",
  "b2",
  "b3",
...
const whiteOutboundPath: Position[] = [
  "w0",
  "w1",
  "w2",
  "w3",
...
export function getCellProperties(
  ...
  if (
    ...
    (position.startsWith("b") || position.startsWith("w"))
  ) {
...
export function getPieceAtPosition(
  ...
  if (stack && stack.pieces.length > 0) {
    ...
    if (
      ...
      stack.pieces[stack.pieces.length - 1].id === movingPieceId
    ) {
...
export function calculateNextPosition(
  ...
  if (journey === "return") {
    ...
    if (currentIndex === -1) {
      console.error(
        "Piece on return journey not found in its return path:",
        currentPosition,
        player,
      );
    ...
    if (currentIndex === -1 && currentPosition !== -1) {
      console.error(
        "Piece not found on expected outbound path:",
        currentPosition,
        player,
      );
...
export function calculateValidMoves(gameState: GameState): Move[] {
  ...
  for (const piece of movablePieces) {
    ...
    const targetJourney = startsReturn
      ? "return"
      : (piece.journey ?? "outbound"); // If starting return, use 'return', else keep current or assume 'outbound' if starting
    ...
    if (targetCellProps.isStackable) {
      ...
      const currentStackSize = stack.pieces.filter(
        (p) => p.id !== piece.id,
      ).length; // Count pieces already there (excluding the moving one if it started there)
...
export function applyMove(gameState: GameState, move: Move): GameState {
  ...
  if (startStack) {
    ...
    startStack.pieces = startStack.pieces.filter(
  ...
  if (targetPosition !== 99) {
    // Don't update stacks if piece exited
    const targetCellProps = getCellProperties(
      targetPosition,
      movingPiece.journey ?? undefined,
    ); // Use updated journey

    if (targetCellProps.isStackable) {
      // Add piece to the target stack
      const targetStack = nextStacks[targetPosition] ?? {
        pieces: [],
        owner: null,
      };
  ...
  const finalPieces = nextPieces.filter(
    (p) => !nextStacks[p.position]?.pieces.some((sp) => sp.id === p.id),
  );
...
export function determineNextStateAfterMove(
  gameState: GameState,
  move: Move,
): Pick<GameState, "status" | "currentPlayer" | "winner" | "message"> {
  ...
  return {
    ...
    winner:
      nextStatus === "black_wins"
        ? "black"
        : nextStatus === "white_wins"
          ? "white"
          : null,
    message: message,
  };
