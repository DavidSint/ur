...
import { GameState, Move, Piece, Position } from "../gameTypes";
...
const positionToGridMap: Partial<Record<Position, [number, number]>> = {
  ...
  w12: [0, 7],
  w11: [0, 6], // Corrected white side positions
  // Black side track (row 2)
  b12: [2, 7],
  b11: [2, 6], // Corrected black side positions
};
...
const Board: React.FC<BoardProps> = ({
  ...
  const getPiecesOnCell = (position: Position | null): Piece[] => {
    ...
    if (
      ...
      if (animatingPiece) {
        ...
        const piecesNormallyAtStart = (
          piecesByPosition.get(position) ?? []
        ).filter((p) => p.id !== animatingPieceId);
    ...
    if (status === "animating" && animatingPieceId !== null) {
      return (piecesByPosition.get(position) ?? []).filter(
        (p) => p.id !== animatingPieceId,
      );
    }
  ...
  const possibleStartPositions =
    status === "moving" ? validMoves.map((m) => m.startPosition) : [];
  const possibleEndPositions =
    status === "moving" ? validMoves.map((m) => m.endPosition) : [];

  // Get off-board pieces using the memoized map
  const blackOffBoard =
    piecesByPosition.get(-1)?.filter((p) => p.player === "black") ?? [];
  const whiteOffBoard =
    piecesByPosition.get(-1)?.filter((p) => p.player === "white") ?? [];

  const canPlayerInteract =
    gameState.status === "rolling" || gameState.status === "moving";
  const isPlayerTurn =
    gameState.currentPlayer === "black" || gameState.gameMode === "twoPlayer"; // Black always controls in vsAI, both in twoPlayer
    ...
    <div className="board">
      <div className="off-board-area">
        <div className="white-start">
          ...
          <div className="piece-container">
            {whiteOffBoard.map((piece) => {
              const moveForThisPiece =
                status === "moving"
                  ? validMoves.find(
                      (m) => m.pieceId === piece.id && m.startPosition === -1,
                    )
                  : undefined;
        ...
        {gameState.gameMode === "twoPlayer" && (
          <Dice
            diceRoll={
              gameState.currentPlayer === "white" ? gameState.diceRoll : null
            }
            ...
            disabled={
              ...
              !!gameState.winner ||
              !canPlayerInteract
            } // Disable if not player's turn to roll or game over
          />
        )}
      </div>
          ...
          <div key={rowIndex} className="board-row">
            {row.map((position, colIndex) => {
              ...
              const isPossibleStart =
                position !== null && possibleStartPositions.includes(position);
              const isPossibleEnd =
                position !== null && possibleEndPositions.includes(position);
      ...
      <div className="off-board-area">
        ...
        <Dice
          diceRoll={
            gameState.currentPlayer === "black" ? gameState.diceRoll : null
          }
          onRollDice={handleRollDice}
          disabled={
